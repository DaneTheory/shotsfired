...
//////////////////////////////////////////////// PANEL GRAPHS /////////////////////////////////////////////////
  ...
  PanelView.prototype._renderMap = function() {
    ...
    this.parent.renderMap(out, (function(v) {
      ...
      return data.properties[_this.config.map.geo.name_field];
    });
    return this;
  };
...
//everything below is as it should be.  commenting out and replacing with code from mexico.js to try to get the multi line hover box.
//seriously.  first is the mexico.js copy and then below, commented out is the code that works, but only conforms to crosslet default hover box.

//start mexico.js copy/paste
//  function MapView() {
//     this._renderMap = __bind(this._renderMap, this);
...
//        _this.path = d3.geo.path().projection(_this.project).pointRadius(7);
...
//begin old working single line .hover code.
  ...
  MapView.prototype.initialize = function(el, config) {
    ...
    $(this.el).attr("class", "crosslet");
    this.map = L.map(el[0]).setView(this.config.map.view.center, this.config.map.view.zoom);
        //change the l.tilelayer below to sort the tiles
    L.tileLayer("https://stamen-tiles-{s}.a.ssl.fastly.net/toner/{z}/{x}/{y}.png", this.config.map.leaflet).addTo(this.map);
...
// method that we will use to update the control based on feature properties passed
info.update = function (props) {
    this._div.innerHTML = '<h1>Shots Fired &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;  2009 - 2013</h1>' +  
                           'Each point on this map represents a Columbia citizen <br/>calling 911 to report hearing' +
' a gunshot. The color of <br/>each dot reflects the range of data contained'+ 
                          ' in the <br/>charts. Interact with the map by' +
        ' clicking and dragging <br/>on the graphs to filter the points<br/>';
};
...
// so I can use <br> inside the tooltip
      ...
      function mousemove(d){
          tooltip
              ...
              .style("top", d3.event.pageY + "px")
              .style("left", d3.event.pageX + "px")
              .html((d.properties.nature == ""?"": d.properties.nature + "<br\>") + 
                    "Time: " + d.properties.time + "<br/>" +
                    "Date: " +d.properties.date + "<br\>" +
                    "Address: " + d.properties.addr);  /* \n does not work in
                                                                          creating a line break*/
      }
    ...
    return this.ds.loadGeo(this.geoURL, this.config.map.geo.id_field, function(ds) {
      _this.bounds = _this.ds.bounds;
      _this.path = d3.geo.path().projection(_this.project).pointRadius(4);
      _this.svg = d3.select(_this.map.getPanes().overlayPane).append("svg");
      _this.g = _this.svg.append("g");
      _this.g.attr("class", "crosslet_geometry");
      
//this is orig block of code for crosslet.js.
      ...
      // }).on("mousemove", _this.mouseMove);
       ...
       //copied and pasted this whole block from mexico- seems they have commented out the top and reformatted to an end, but not sure what that is- seems to be working on tooltip, but not sure.
...
.attr("id", function(d) {
      ...
      //comment out next three lines to refelct mexico.
      _this.hoverElement = _this.svg.append("g").attr("class", "hover");
      _this.hoverElementRect = _this.hoverElement.append("svg:rect").attr("x", 0).attr("y", 0).attr("width", 10).attr("height", 30).attr("rx", 5).attr("ry", 5);
      _this.hoverElementText = _this.hoverElement.append("text").attr("x", 0).attr("y", 0);
      _this.hoverElementTextBB = {
  ...
  MapView.prototype.default_hover = function(data) {
    return data.properties[this.config.map.geo.name_field] ;
  };
